...
def predict(image):
    if image is None:
        return "Upload an MRI image first.", {}, ""
    ...
    return summary, probability_map, prediction


with gr.Blocks(title="NeuroScan AI") as demo:
    gr.Markdown(
        "# NeuroScan AI\n"
        "Upload an MRI slice and the model will classify it as Normal or Tumor."
    )
    with gr.Row():
        image_input = gr.Image(type="pil", label="MRI Image")
        with gr.Column():
            summary_output = gr.Textbox(label="Result", lines=2)
            probabilities_output = gr.JSON(label="Probabilities")
            prediction_output = gr.Textbox(label="Predicted Class")
    submit_button = gr.Button("Analyze Scan")
    submit_button.click(
        fn=predict,
        inputs=image_input,
        outputs=[summary_output, probabilities_output, prediction_output],
    )
